...
PARAMS: List[Tuple[str, List[Param]]] = [
    ("在場型態(經過 / 徘徊 / 等待)", [
        ...
        Param(("presence", "path_dt"), "路徑重取樣間隔", 0.1, 2.0, 0.1, "秒",
              "累積路徑每隔多久取一個樣。逐幀相加會讓偵測抖動線性累積:"
              "實測靜止的人抖動 5px 就能累到 3.85 身高(徘徊門檻 3.0),"
              "而且取樣率 5→30 fps 會讓數值從 1.18 變 7.12"),
        Param(("presence", "path_deadband"), "路徑死區", 0.0, 0.3, 0.01, "身高",
              "單步位移小於此值視為抖動,不計入累積路徑"),
        Param(("state_machine", "sequence_bonus"), "完整序列加成", 0.0, 0.5,
              0.05, "",
              "視窗內出現完整 S1→S2→S3 時規則側分數的加成。做成加分而非"
              "必要條件:S1 常常漏報,當門檻會大量漏測",
              needs="skeleton"),
    ...
    ("錄影", [
        Param(("alarm", "clip_overlay"), "警報片段疊加骨架", 0, 1, 1, "",
              ...
              boolean=True),
        Param(("alarm", "clip_pre_sec"), "片段保留觸發前幾秒", 5, 60, 1, "秒",
              "標記指向促成第一次計入事件的那次抬手,而抬手到警報成立常隔"
              "十幾秒。這個值小於那段間隔時,標記跳得到但片段拍不到動作的"
              "開頭。調大要付記憶體:滾動緩衝每秒約 10 張取樣影格"),
        Param(("alarm", "count_driven"), "次數達標即推向觸發", 0, 1, 1, "",
              "開:次數湊滿就把分數拉到 count_floor,確保會通報。關:分數"
              "完全交給融合結果——要比較「加了外觀網路有沒有差」時建議關掉,"
              "開著時兩者的警報時刻會非常接近", boolean=True),
        Param(("alarm", "count_floor"), "次數達標時的分數下限", 0.3, 1.0, 0.05,
              "", "次數達標時把單週期分數拉到這個下限。設成 1.0 等於丟掉融合"
              "結果(舊行為),設在觸發線稍上方則保留方法之間的差異"),
    ]),
    ("移動排除(走動中不判抽菸)", [
